...
app.post('/api/admin/leads/:id/email', authenticateAdmin, [
    ...
    try {
        ...
        const fromAddr = adminWorkEmail
            ? `${adminWorkEmail} <${process.env.EMAIL_USER}>`
            : `LaunchPoint DM <${process.env.EMAIL_USER}>`;

        await transporter.sendMail({
            from: fromAddr,
            replyTo: adminWorkEmail || process.env.EMAIL_USER,
...
// POST /api/admin/leads/:id/sms - send SMS via Vonage
app.post('/api/admin/leads/:id/sms', authenticateAdmin, [
    ...
    try {
        ...
        if (!vonage) {
            return res.status(503).json({ error: 'SMS service not configured. Add VONAGE_API_KEY and VONAGE_API_SECRET to your .env file.' });
        }
