...
// Serve Vue SPA build output (dist/) — only when it exists
...
// Serve the Vue SPA entry when dist/ is built, otherwise fall back to legacy static HTML
function serveSpa(legacyFile) {
    return (req, res) => {
        if (fs.existsSync(distIndex)) return res.sendFile(distIndex);
        const legacy = publicHtml(legacyFile);
...
// Serve main pages
app.get('/',         logAnalytics('page_view'), serveSpa('index.html'));
app.get('/about',    logAnalytics('page_view'), serveSpa('about.html'));
app.get('/platform', logAnalytics('page_view'), serveSpa('platform.html'));
app.get('/events',   logAnalytics('page_view'), serveSpa('events.html'));
app.get('/contact',  logAnalytics('page_view'), serveSpa('index.html'));

app.get('/volunteer', logAnalytics('page_view'), (req, res) => {
    const f = publicHtml('volunteer.html');
    if (fs.existsSync(f)) return res.sendFile(f);
    res.redirect('/');
});

app.get('/donate', logAnalytics('page_view'), (req, res) => {
    const f = publicHtml('donate.html');
    if (fs.existsSync(f)) return res.sendFile(f);
    res.redirect('/');
});
...
const SERVICE_PRICING = {
    ...
    website_update:       { label: 'Website Updates/Redesign', type: 'one_time', base: 2200  },
    ...
    fundraising:            { label: 'Fundraising Strategy',    type: 'monthly',  base: 2000  }
};
...
app.post('/api/contact', 
    ...
    [
        ...
        body('email').isEmail().normalizeEmail(),
        body('phone').optional().isMobilePhone(),
        body('message').trim().isLength({ min: 4, max: 2000 }),
...
async function sendApprovalEmail(to, fullName, username, tempPassword) {
    ...
    <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;background:#0f1729;color:#e2e8f0;padding:40px;border-radius:16px">
        <div style="text-align:center;margin-bottom:32px">
            ...
            <h1 style="color:#4db8ff;margin:0">Blue Ocean Strategies</h1>
        ...
        <p>Welcome to the team!<br>— Blue Ocean Strategies</p>
        <p style="color:#64748b;font-size:11px;margin-top:32px;border-top:1px solid #1e2d4a;padding-top:16px">Blue Ocean Strategies — Political Marketing Platform</p>
    </div>`;
    ...
    if (process.env.EMAIL_USER) {
        try {
            await transporter.sendMail({
                from: `"Blue Ocean Strategies" <${process.env.EMAIL_USER}>`,
                to,
                subject: '✅ Your BOS Admin Access Has Been Approved',
                html
            });
        } catch (e) {
...
async function sendRejectionEmail(to, fullName, reason) {
    ...
    <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;background:#0f1729;color:#e2e8f0;padding:40px;border-radius:16px">
        ...
        <p>Hello ${fullName},</p>
        <p>Thank you for your interest in the Blue Ocean Strategies admin portal. Unfortunately, your request for access was <strong style="color:#e74c3c">not approved</strong> at this time.</p>
        ${reason ? `<p><strong>Reason:</strong> ${reason}</p>` : ''}
        <p>If you believe this is an error, please contact your campaign manager.</p>
        <p>— Blue Ocean Strategies</p>
    </div>`;
    ...
    if (process.env.EMAIL_USER) {
        try {
            await transporter.sendMail({
                from: `"Blue Ocean Strategies" <${process.env.EMAIL_USER}>`,
                to,
                subject: 'BOS Admin Access Request Update',
                html
            });
        } catch (e) {
