...
async function initializeDatabase() {
    ...
    try {
        ...
        `);
        await client.query(`ALTER TABLE admin_registrations ADD COLUMN IF NOT EXISTS phone TEXT`);
        ...
        // Work email issued on approval
        await client.query(`ALTER TABLE admin_users ADD COLUMN IF NOT EXISTS work_email TEXT`);
...
async function clickSendSms(to, text) {
    if (!process.env.CLICKSEND_USERNAME || !process.env.CLICKSEND_API_KEY) {
        throw new Error('SMS service not configured. Add CLICKSEND_USERNAME and CLICKSEND_API_KEY to your .env file.');
    }
    ...
    const response = await fetch('https://rest.clicksend.com/v3/sms/send', {
        ...
        body: JSON.stringify({
            messages: [{
                ...
                body:    text,
                to:      to,
                from:    process.env.CLICKSEND_FROM || 'LaunchPoint'
            }]
        })
    });
    const data = await response.json();
    if (data.http_code !== 200) {
        throw new Error(data.response_msg || 'ClickSend error');
    }
    return data;
}

async function sendSmsAlert(text) {
    if (!process.env.CLICKSEND_USERNAME || !process.env.CLICKSEND_TO_NUMBER) return;
    try {
        await clickSendSms(process.env.CLICKSEND_TO_NUMBER, text);
    } catch (err) {
        console.error('ClickSend SMS alert error:', err.message);
    }
}
        ...
        try {
            ...
            if (process.env.RESEND_API_KEY) {
                ...
                const mailOptions = {
                    ...
                    to: process.env.NOTIFICATION_EMAIL || 'hello@launchpoint-dm.com',
            ...
            // SMS notification via ClickSend — alert superadmin
            await sendSmsAlert(`New lead: ${firstName} ${lastName} | ${email}${phone ? ' | ' + phone : ''} | Est. $${estimate.total.toLocaleString()}`);

            // SMS confirmation to the lead if they provided a phone number
            if (phone) {
                try {
                    await clickSendSms(phone.replace(/[^\d+]/g, ''), `Hi ${firstName}, thanks for reaching out to LaunchPoint DM! We received your message and will follow up within 1 business day. — LaunchPoint DM`);
                } catch (smsErr) {
                    console.error('Lead SMS confirmation error:', smsErr.message);
                }
            }
...
app.post('/api/newsletter', 
    ...
    async (req, res) => {
        ...
        try {
            ...
            // Notify superadmin by SMS
            await sendSmsAlert(`[LaunchPoint DM] New newsletter subscriber: ${firstName || ''} ${lastName || ''} <${email}>`);
            // Send welcome confirmation email to subscriber
            if (process.env.RESEND_API_KEY) {
                try {
                    await transporter.sendMail({
                        from: process.env.RESEND_FROM || 'LaunchPoint DM <hello@launchpoint-dm.com>',
                        to: email,
                        subject: 'Welcome to the LaunchPoint DM Growth Insights Brief!',
                        html: `<div style="font-family:Arial,sans-serif;max-width:580px;margin:0 auto;background:#0f1729;color:#e2e8f0;padding:36px;border-radius:14px">
                            <div style="text-align:center;margin-bottom:28px">
                                <div style="font-size:42px;margin-bottom:10px">🚀</div>
                                <h1 style="color:#4db8ff;margin:0;font-size:24px">LaunchPoint DM</h1>
                                <p style="color:#94a3b8;margin-top:8px">Growth Insights Brief</p>
                            </div>
                            <p>Hi${firstName ? ' ' + firstName : ''},</p>
                            <p>You're in! We'll be sending you digital marketing insights, strategy tips, and campaign ideas straight to your inbox.</p>
                            <p style="color:#94a3b8;font-size:13px">If you didn't sign up for this, you can safely ignore this email.</p>
                            <p>— The LaunchPoint DM Team</p>
                            <p style="color:#64748b;font-size:11px;margin-top:28px;border-top:1px solid #1e2d4a;padding-top:14px">LaunchPoint DM · <a href="https://launchpoint-dm.com" style="color:#4db8ff">launchpoint-dm.com</a></p>
                        </div>`
                    });
                } catch (mailErr) {
    ...
    async (req, res) => {
        try {
            ...
            res.json({ 
                success: true, 
                token,
                user: { id: user.id, username: user.username, email: user.email, role: user.role, work_email: user.work_email || null }
            });
        } catch (error) {
    ...
    try {
        const result = await pool.query(
            `SELECT first_name, last_name, email, phone, interest, message,
                    ...
                    COALESCE(source_website, 'LaunchPoint DM Website') as source_website,
        ...
        result.rows.forEach(row => {
            csv += [
                ...
                `"${row.submission_date}"`, `"${row.source_website || 'LaunchPoint DM Website'}"`,
...
app.post('/api/admin/leads/:id/email', authenticateAdmin, [
    ...
    try {
        ...
        if (!process.env.RESEND_API_KEY) {
            return res.status(503).json({ error: 'Email service not configured. Add RESEND_API_KEY to your .env file.' });
        }

        // Use the admin's work email as the From address if available
        const adminRow = (await pool.query('SELECT work_email FROM admin_users WHERE username = $1', [req.user.username])).rows[0];
        const adminWorkEmail = adminRow?.work_email;
        const fromAddr = adminWorkEmail
            ? adminWorkEmail
            : (process.env.RESEND_FROM || 'LaunchPoint DM <hello@launchpoint-dm.com>');

        await transporter.sendMail({
            from: fromAddr,
            replyTo: adminWorkEmail || process.env.RESEND_FROM,
...
// POST /api/admin/leads/:id/sms - send SMS via ClickSend
app.post('/api/admin/leads/:id/sms', authenticateAdmin, [
    ...
    try {
        ...
        await clickSendSms(lead.phone.replace(/[^\d+]/g, ''), smsBody);
...
app.get('/api/admin/stats', authenticateAdmin, async (req, res) => {
    try {
        const [tot, tod, newL, cont, conv, subs, recent, byStatus, bySource] = await Promise.all([
            ...
            pool.query(`SELECT COALESCE(source_website, 'LaunchPoint DM Website') as source, COUNT(*) as count FROM contact_submissions GROUP BY source_website ORDER BY count DESC`)
        ]);
        res.json({
            stats: {
...
function generateWorkEmail(fullName) {
    const parts = (fullName || '').trim().split(/\s+/);
    const first = (parts[0] || 'admin').toLowerCase().replace(/[^a-z]/g, '');
    const last  = (parts[parts.length - 1] || '').toLowerCase().replace(/[^a-z]/g, '');
    const firstInitial = first.charAt(0);
    return `${first}.${firstInitial}${last}@launchpoint-dm.com`;
}
...
async function sendApprovalEmail(to, fullName, username, tempPassword, workEmail) {
    ...
    <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;background:#0f1729;color:#e2e8f0;padding:40px;border-radius:16px">
        <div style="text-align:center;margin-bottom:32px">
            ...
            <p style="color:#94a3b8;margin-top:8px">Admin Access Approved</p>
        </div>
        <p>Hello ${fullName},</p>
        ...
        <div style="background:#1a2540;border:1px solid #2d3a5c;border-radius:12px;padding:20px;margin:24px 0">
            ...
            <p style="margin:0 0 10px"><strong>Temporary Password:</strong> <code style="background:#0f1729;padding:2px 8px;border-radius:4px;color:#4db8ff;font-size:15px">${tempPassword}</code></p>
            <p style="margin:0"><strong>Your Work Email:</strong> <code style="background:#0f1729;padding:2px 8px;border-radius:4px;color:#f4a832;font-size:15px">${workEmail || 'Assigned upon setup'}</code></p>
        </div>
        <p style="color:#f39c12">⚠️ Please log in and change your password immediately after first login.</p>
        <p style="font-size:13px;color:#94a3b8;">Your work email <strong style="color:#f4a832">${workEmail || ''}</strong> is your official LaunchPoint DM address. All client communications will use this address.</p>
...
async function sendRejectionEmail(to, fullName, reason) {
    ...
    <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;background:#0f1729;color:#e2e8f0;padding:40px;border-radius:16px">
        ...
        <p>Hello ${fullName},</p>
        <p>Thank you for your interest in the LaunchPoint DM admin portal. Unfortunately, your request for access was <strong style="color:#e74c3c">not approved</strong> at this time.</p>
...
app.post('/api/admin/register',
    ...
    async (req, res) => {
        const errors = validationResult(req);
        if (!errors.isEmpty()) return res.status(400).json({ errors: errors.array() });
        const { fullName, email, phone, organization, reason } = req.body;
        try {
            ...
            await pool.query(
                `INSERT INTO admin_registrations (full_name, email, phone, organization, reason) VALUES ($1,$2,$3,$4,$5)`,
                [fullName, email, phone || null, organization || null, reason || null]
            );
            // Notify superadmin by SMS
            await sendSmsAlert(`[LaunchPoint DM] New admin registration request from ${fullName} (${email}${phone ? ', ' + phone : ''}). Log in to review.`);
...
app.post('/api/superadmin/registrations/:id/approve', authenticateSuperAdmin, async (req, res) => {
    ...
    try {
        ...
        const { full_name, email, phone } = reg.rows[0];
        const username = generateUsername(full_name);
        const workEmail = generateWorkEmail(full_name);
        const tempPassword = generateTempPassword();
        const passwordHash = await bcrypt.hash(tempPassword, 10);

        await pool.query(
            `INSERT INTO admin_users (username, email, password_hash, role, active, work_email) VALUES ($1,$2,$3,'admin',TRUE,$4)`,
            [username, email, passwordHash, workEmail]
        );
        await pool.query(
        ...
        await sendApprovalEmail(email, full_name, username, tempPassword, workEmail);
        // Notify superadmin by SMS
        await sendSmsAlert(`[LaunchPoint DM] Admin approved: ${full_name} (${username}). Work email: ${workEmail}.`);
...
app.post('/api/superadmin/registrations/:id/reject', authenticateSuperAdmin, [
    ...
    try {
        ...
        await sendSmsAlert(`[LaunchPoint DM] Admin request rejected: ${reg.rows[0].full_name} (${reg.rows[0].email}).${reason ? ' Reason: ' + reason : ''}`);
        // SMS the applicant if they provided a phone number
        if (reg.rows[0].phone) {
            try {
        ...
        const [leadsResult, countResult, adminsResult] = await Promise.all([
            pool.query(
                `SELECT cs.id, cs.name, cs.email, cs.phone, cs.status, cs.assigned_to,
                        cs.submission_date, cs.service_interest, cs.source_website AS source_url
