// Political Marketing Platform - Node.js Server
// Advanced Dynamic Platform for Political Campaigns
...
// Database Setup
const db = new sqlite3.Database('political_marketing.db', (err) => {
    if (err) {
...
app.get('/api/admin/export/contacts', authenticateAdmin, (req, res) => {
    ...
    `, [], (err, rows) => {
        ...
        const headers = [
            ...
            'Message', 'Newsletter', 'Date', 'UTM Source', 'UTM Medium', 'UTM Campaign'
        ];
        ...
        rows.forEach(row => {
            csv += [
                ...
                row.interest || '', `"${row.message}"`, row.newsletter_signup ? 'Yes' : 'No',
...
// Start server
app.listen(PORT, () => {
    console.log(`🚀 Political Marketing Platform running on port ${PORT}`);
